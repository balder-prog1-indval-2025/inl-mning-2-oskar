...
let  h=-150
let woman3 = await fetchImage ("OIP3.jpg")
...
let framme = false
export function lukazattack(död1){
...
let vänster = true
export async function marrekuben(speed: number,x2: number,död1 ){
...
let womanv = true
export function woman(woman,död2){
...
export function woman9(woman,död2){
    let hitboxwoman = new Hitbox(woman.x,woman.y,75,200)
    hitboxwoman.x = woman.x
    hitboxwoman.y = woman.y
    if (womanv && woman.x < woman.h){
...
export function reset(){
 ...
 export function ippeboss1(ippebossbana2){
...
export function ippeboss(död2,winner){
...
export function ippetomat(ippe,död2){
...
export function coins1(x: number, y: number,coinvalue){
...
export function coins2(x: number, y: number,coinvalue){
    ctx.drawImage(coin, x, y,50,60)
    let coinhb = new Hitbox(x,y,50,60)
    if (hitbox.intersects(coinhb) ){
    return(coinvalue=2)
...
export function coins3(x: number, y: number,coinvalue){
    ctx.drawImage(coin, x, y,50,60)
    let coinhb = new Hitbox(x,y,50,60)
    if (hitbox.intersects(coinhb) ){
    return(coinvalue=3)
...
export function death1(bana1,bana2,bana3,ippebossbana1,ippebossbana2,död1): void{
...
export function death2(bana1,bana2,bana3,ippebossbana1,ippebossbana2,död2): void{
    rectangle(0,0,10000,10000)
    text('WASTED', W - W / 2 - 475, 400, 300, 'red')
    text('Press Enter to restart', W  - 375, H-30, 25, 'red')
    dödsljudet.play();
